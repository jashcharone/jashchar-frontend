...
const EditArticle = () => {
    ...
    const fetchArticle = useCallback(async () => {
        ...
        const { data, error } = await supabase.from('articles').select('*, master_admin_profiles(full_name, email)').eq('id', id).single();
        if (error) {
            ...
            setMetaKeywords((data.meta_keywords || []).join(', '));
    ...
    const handleSubmit = async (e) => {
        ...
        if (error) {
            toast({ variant: 'destructive', title: 'Error updating article.', description: error.message });
        } else {
                    ...
                    <div className="space-y-6">
                        <Card>
                            ...
                            <CardContent className="space-y-4">
                                ...
                                <div>
                                    <Label>Author</Label>
                                    <Input value={article?.master_admin_profiles?.full_name || article?.master_admin_profiles?.email} disabled />
                                </div>
                                <div>
