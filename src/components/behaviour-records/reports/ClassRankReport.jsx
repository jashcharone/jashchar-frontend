...
const ClassRankReport = ({ fetchData, loading }) => {
  const [reportData, setReportData] = useState([]);
  const [showModal, setShowModal] = useState(false);
  const [selectedData, setSelectedData] = useState(null);
  
  // Auto-load data on mount or we could add a search button if preferred
  useEffect(() => {
    handleSearch();
  }, []);
  ...
  const handleSearch = async () => {
    ...
    data.forEach(record => {
      if (!record.student?.class?.id) return;
    ...
    let aggregated = Array.from(classMap.values()).map(item => ({
      ...item,
  ...
  const handleShowDetails = (entry) => {
    setSelectedData({
      title: `Incidents for Class: ${entry.name}`,
      data: entry.records
    });
    setShowModal(true);
  };
...
export default ClassRankReport;
