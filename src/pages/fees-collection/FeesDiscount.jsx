...
const FeesDiscount = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        let error;
        if (editingDiscount) {
            ({ error } = await supabase.from('fee_discounts').update(dataToSubmit).eq('id', editingDiscount.id));
        } else {
            ({ error } = await supabase.from('fee_discounts').insert(dataToSubmit));
        }
    ...
    const handleDelete = async (discountId) => {
        ...
        const { error } = await supabase.from('fee_discounts').delete().eq('id', discountId);
        if (error) {
    ...
    const handleAssignSave = async () => {
        ...
        // Delete existing assignments for this discount
        ...
        <DashboardLayout>
            ...
            <Dialog open={assignModalOpen} onOpenChange={setAssignModalOpen}>
                <DialogContent className="max-w-4xl">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-6 gap-4 my-4 p-4 border rounded-lg items-end">
                        <div className="md:col-span-2"><Label>Class</Label><Select onValueChange={(v) => setFilters(p => ({...p, class_id: v}))}><SelectTrigger><SelectValue placeholder="Select Class" /></SelectTrigger><SelectContent>{classes.map(c => <SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}</SelectContent></Select></div>
                        <div className="md:col-span-2"><Label>Section</Label><Select onValueChange={(v) => setFilters(p => ({...p, section_id: v}))}><SelectTrigger><SelectValue placeholder="Select Section" /></SelectTrigger><SelectContent>{sections.map(s => <SelectItem key={s.id} value={s.id}>{s.name}</SelectItem>)}</SelectContent></Select></div>
                        <div className="md:col-span-2"><Button className="w-full" onClick={handleSearchStudents}>Search</Button></div>
                    </div>
