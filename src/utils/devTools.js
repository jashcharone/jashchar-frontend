// ðŸ§  Jashchar Debug Helper Toolkit
...
export const debug = (label, data) => {
  try {
    ...
    console.log(`%cðŸ§© ${label}:`, "color:#3b82f6; font-weight:bold;", value);
  } catch (err) {
    console.log(` ï¸ ${label} (error parsing):`, data);
  }
};
...
export const checkAuth = async () => {
  console.group("%cðŸŸ¢ Supabase Auth Check", "color:green; font-weight:bold;");
  const { data, error } = await supabase.auth.getSession();
  if (error) console.error("Auth error:", error);
  else if (data?.session) {
    console.log("✅ Session Active:", data.session);
    console.log("ðŸ‘¤ User:", data.session.user);
  } else console.warn(" ï¸ No active User Session (Not Logged In).");
  console.groupEnd();
};
...
export const initDevTools = () => {
  console.log("%cðŸš€ DevTools Initialized ” Debug Mode ON", "color:#22c55e; font-weight:bold;");
  checkAuth();
  performanceStats();
};
