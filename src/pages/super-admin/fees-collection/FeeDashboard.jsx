          ...
          <CardContent className="p-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-sm font-medium text-green-700 uppercase tracking-wider">Today&apos;s Collection</p>
                  <p className="text-4xl font-bold text-green-800">{formatCurrency(todayStats.collectedToday)}</p>
                </div>
              </div>
              <div className="flex gap-6">
                <div className="text-center">
                  <p className="text-3xl font-bold text-green-700">{todayStats.paymentsToday}</p>
                  <p className="text-sm text-green-600">Payments</p>
                </div>
                <Separator orientation="vertical" className="h-14" />
                <div className="text-center">
                  <p className="text-3xl font-bold text-green-700">{todayStats.studentsToday}</p>
                  ...
                  {paymentModes.length === 0 ? (
                    ...
                    paymentModes.map((pm, idx) => (
                      <div key={idx} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        <div className="flex items-center gap-3">
                          {pm.mode === 'Cash' && <Wallet className="h-5 w-5 text-green-600" />}
                          {pm.mode === 'Online' && <CreditCard className="h-5 w-5 text-blue-600" />}
                          {pm.mode === 'UPI' && <Zap className="h-5 w-5 text-purple-600" />}
                          {!['Cash', 'Online', 'UPI'].includes(pm.mode) && <Banknote className="h-5 w-5 text-gray-600" />}
