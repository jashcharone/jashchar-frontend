...
const PostalReceive = () => {
  ...
  const [formData, setFormData] = useState({
    from_title: '', reference_no: '', address: '', note: '', to_title: '', 
    date: new Date().toISOString().split('T')[0], document_url: '', id: null
  });

  useEffect(() => {
    if (branchId) fetchReceives();
  }, [branchId]);

  const fetchReceives = async () => {
    setLoading(true);
    const { data, error } = await supabase.from('postal_receive').select('*').eq('branch_id', branchId).order('created_at', { ascending: false });
    if (error) toast({ variant: 'destructive', title: 'Error', description: error.message });
    else setReceives(data || []);
    setLoading(false);
  };
  ...
  const handleFileUpload = async (e) => {
    ...
    setUploading(true);
    try {
      const fileName = `${branchId}/receive_${Date.now()}_${file.name}`;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let error;
      if (formData.id) {
        ({ error } = await supabase.from('postal_receive').update(payload).eq('id', formData.id));
      } else {
        ({ error } = await supabase.from('postal_receive').insert(payload));
      }

      if (error) throw error;
      toast({ title: 'Saved successfully' });
      setFormData({ from_title: '', reference_no: '', address: '', note: '', to_title: '', date: new Date().toISOString().split('T')[0], document_url: '', id: null });
      fetchReceives();
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    ...
    const { error } = await supabase.from('postal_receive').delete().eq('id', id);
  ...
  const filtered = receives.filter(d => d.from_title.toLowerCase().includes(search.toLowerCase()) || d.reference_no?.includes(search));

  return (
    <DashboardLayout>
      <div className="p-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-1 h-fit">
          <CardHeader><CardTitle>Add Postal Receive</CardTitle></CardHeader>
          <CardContent>
        ...
        <Card className="lg:col-span-2">
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle>Postal Receive List</CardTitle>
            <Input placeholder="Search..." className="max-w-xs" value={search} onChange={e => setSearch(e.target.value)} />
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader><TableRow><TableHead>From Title</TableHead><TableHead>Reference No</TableHead><TableHead>To Title</TableHead><TableHead>Date</TableHead><TableHead className="text-right">Action</TableHead></TableRow></TableHeader>
              <TableBody>
                ...
                : filtered.map(item => (
                  <TableRow key={item.id}>
                    <TableCell className="font-medium">{item.from_title}</TableCell>
