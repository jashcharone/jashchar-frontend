...
const StudentAnalysis = () => {
  ...
  const handleDownloadPDF = async () => {
    ...
    const bodyHtml = `
      ...
      <h2>Class-wise Distribution</h2>
      <table><thead><tr><th>Class</th><th>Total</th><th>Male</th><th>Female</th><th>Other</th><th>Sections</th></tr></thead>
        <tbody>${classWiseData.map(c => `<tr><td>${c.name}</td><td><strong>${c.total}</strong></td><td>${c.male}</td><td>${c.female}</td><td>${c.other}</td><td>${Object.entries(c.sections).map(([s, n]) => `${s}: ${n}`).join(', ')}</td></tr>`).join('')}</tbody></table>
      <h2>Admission Type Distribution</h2>
      <table><thead><tr><th>Category</th><th>Count</th><th>Percentage</th></tr></thead>
        <tbody>${categoryDistribution.map(c => `<tr><td>${c.name}</td><td>${c.count}</td><td>${stats.totalStudents > 0 ? ((c.count / stats.totalStudents) * 100).toFixed(1) : 0}%</td></tr>`).join('')}</tbody></table>
      <h2>Age Distribution</h2>
      <table><thead><tr><th>Age Group</th><th>Count</th><th>Percentage</th></tr></thead>
        <tbody>${ageDistribution.map(a => `<tr><td>${a.range} years</td><td>${a.count}</td><td>${stats.totalStudents > 0 ? ((a.count / stats.totalStudents) * 100).toFixed(1) : 0}%</td></tr>`).join('')}</tbody></table>
      <h2>Fees Overview</h2>
      <div class="stats-grid">
        <div class="stat-box"><div class="stat-value">₹${feesOverview.totalAllocated.toLocaleString('en-IN')}</div><div class="stat-label">Total Allocated</div></div>
        <div class="stat-box"><div class="stat-value text-green">₹${feesOverview.totalPaid.toLocaleString('en-IN')}</div><div class="stat-label">Total Paid</div></div>
    ...
    await downloadReportAsPDF({
      title: `Student Analysis Report - ${selectedBranch?.name || 'School'}`,
      orgHeader,
      bodyHtml,
      fileName: `Student_Analysis_${selectedBranch?.name || 'School'}_${format(new Date(), 'yyyy-MM-dd')}`,
    });
  };
