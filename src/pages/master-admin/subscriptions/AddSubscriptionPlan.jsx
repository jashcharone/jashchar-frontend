...
import { Plus, Save, Users, UserCheck, Loader2 } from 'lucide-react';
...
const AddSubscriptionPlan = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const dataToSubmit = {
      ...
      // modules: formData.modules, // Handled separately via service
    ...
    try {
        // 1. Create Plan in Supabase directly
        ...
        // 2. Save Modules using Service
        if (formData.modules && formData.modules.length > 0) {
            const result = await planModuleService.setModulesForPlan(newPlan.id, formData.modules);
            if (!result.success) {
                console.error("Module save error:", result.error);
                toast({ variant: 'warning', title: 'Plan created but modules failed', description: result.error });
            }
        }
  ...
  return (
    <DashboardLayout>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold">Add Subscription Plan</h1>
         <div className="flex gap-2">
            <Button onClick={() => navigate('/master-admin/subscription-plans')}>
                Package List
            </Button>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        {/* Modules Section - Using Dynamic Loader */}
        <div className="bg-card rounded-xl shadow-lg p-8 border">
        ...
        <div className="mt-6 flex justify-end">
          <Button type="submit" disabled={loading} className="bg-primary hover:bg-primary/90 text-primary-foreground font-bold py-2 px-6 rounded-lg">
