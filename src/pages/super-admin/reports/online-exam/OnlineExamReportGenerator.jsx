...
import { fetchOnlineExamDataFromSupabase } from '../ReportGeneratorShared/reportQueries';
...
const OnlineExamReportGenerator = () => {
  ...
  const selectedColumnsObjects = useMemo(() => {
    return selectedColumns
      .map(key => ONLINE_EXAM_COLUMNS.find(c => c.key === key))
      .filter(Boolean);
  }, [selectedColumns]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const onlineExamData = await fetchOnlineExamDataFromSupabase({
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="border-b border-gray-200 dark:border-gray-700 p-4 space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-4">
              <ColumnSelector
                availableColumns={ONLINE_EXAM_COLUMNS}
