...
const AcademicAnalysis = () => {
  ...
  const handleDownloadPDF = async () => {
    ...
    const bodyHtml = `
      ...
      <h2>Class-wise Academic Details</h2>
      <table><thead><tr><th>Class</th><th>Sections</th><th>Students</th><th>Class Teacher</th><th>Subjects</th><th>Avg/Section</th></tr></thead>
        <tbody>${classDetails.map(c => `<tr><td>${c.name}</td><td>${c.sections.join(', ') || '-'}</td><td><strong>${c.studentCount}</strong></td><td>${c.classTeacher || 'Not Assigned'}</td><td>${c.subjectCount}</td><td>${c.sections.length > 0 ? Math.round(c.studentCount / c.sections.length) : c.studentCount}</td></tr>`).join('')}</tbody></table>
      <h2>Subject Summary</h2>
      <table><thead><tr><th>Subject</th><th>Code</th><th>Type</th><th>Classes Using</th><th>Teachers Assigned</th></tr></thead>
        <tbody>${subjectSummary.map(s => `<tr><td>${s.name}</td><td>${s.code || '-'}</td><td>${s.type}</td><td>${s.classCount}</td><td>${s.teacherCount}</td></tr>`).join('')}</tbody></table>
      <h2>Teacher Workload</h2>
      <table><thead><tr><th>Teacher</th><th>Class Teacher Of</th><th>Subject Assignments</th><th>Total Load</th></tr></thead>
        <tbody>${teacherWorkload.map(t => `<tr><td>${t.name}</td><td>${t.classesHandled}</td><td>${t.subjectsHandled}</td><td>${t.classesHandled + t.subjectsHandled}</td></tr>`).join('')}</tbody></table>
      <h2>Student-Teacher Ratio</h2>
      <table><thead><tr><th>Class</th><th>Students</th><th>Teachers</th><th>Ratio</th></tr></thead>
        <tbody>${studentTeacherRatio.map(r => `<tr><td>${r.class}</td><td>${r.students}</td><td>${r.teachers}</td><td>${r.ratio}</td></tr>`).join('')}</tbody></table>
      <div class="footer">Generated by Jashchar ERP • ${format(new Date(), 'dd MMMM yyyy, hh:mm a')}</div>`;
    await downloadReportAsPDF({
      title: `Academic Analysis Report - ${selectedBranch?.name || 'School'}`,
      orgHeader,
      bodyHtml,
      fileName: `Academic_Analysis_${selectedBranch?.name || 'School'}_${format(new Date(), 'yyyy-MM-dd')}`,
    });
  };
