...
const LiveFaceAttendance = () => {
                ...
                try {
                    ...
                    if (isHealthy) {
                        ...
                        try {
                            ...
                            if (totalFaces === 0) {
                                console.log('?? AI Engine healthy but FAISS index empty — click "Sync to AI" to populate');
                    ...
                    console.log('?? AI Engine not available, using browser AI');
                    setAIEngineAvailable(false);
                }
    ...
    const fetchClassStudents = async () => {
        ...
        let query = supabase
            ...
            .select('id, full_name, enrollment_id, photo_url')
            .eq('branch_id', branchId)
    ...
    const syncFacesToAIEngine = async () => {
        ...
        try {
            ...
            const faces = data || [];
            if (faces.length === 0) {
                toast({ title: '?? No faces to sync', description: 'Register faces first before syncing.' });
                setSyncing(false);
                return;
            }
