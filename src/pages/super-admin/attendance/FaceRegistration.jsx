    ...
    const handleSaveFace = async () => {
        ...
        try {
            ...
            const payload = {
                ...
                is_active: true,
                quality_score: Math.min(100, capturedPhotos.length * 33),
                photo_count: capturedPhotos.length,
                // In production: face_encoding would be generated by ML model
                metadata: {
                    captured_at: new Date().toISOString(),
                    device: 'webcam',
                    angles: capturedPhotos.map(p => p.angle),
                },
            };
            ...
            toast({ 
                ...
                description: `${capturedPhotos.length} photos captured for ${selectedUser[searchType === 'student' ? 'student_name' : 'staff_name']}`,
            });
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="font-semibold">
                                                {searchType === 'student' ? selectedUser?.student_name : selectedUser?.staff_name}
                                            </p>
                                            <p className="text-sm text-muted-foreground">
...
const FaceRegistration = () => {
    ...
    const fetchRegistrations = async () => {
        ...
        const { data, error } = await supabase
            ...
            .select(`
                *,
                student:student_profiles!face_encodings_user_id_fkey(student_name, admission_no, class_name, section, photo_url),
                staff:staff_profiles!face_encodings_user_id_fkey(staff_name, employee_id, department, photo_url)
            `)
            .eq('branch_id', branchId)
        ...
        if (error) {
            ...
            const processedData = (data || []).map(reg => ({
                ...reg,
                user_name: reg.user_type === 'student' 
                    ? reg.student?.student_name 
                    : reg.staff?.staff_name,
                user_code: reg.user_type === 'student'
                    ? reg.student?.admission_no
                    : reg.staff?.employee_id,
                user_detail: reg.user_type === 'student'
                    ? `${reg.student?.class_name || ''} ${reg.student?.section || ''}`
                    : reg.staff?.department || '',
                photo_url: reg.user_type === 'student'
                    ? reg.student?.photo_url
                    : reg.staff?.photo_url,
            }));
    ...
    const filteredRegistrations = registrations.filter(reg => {
        ...
        const matchesType = filterType === 'all' || reg.user_type === filterType;
    ...
    const stats = {
        ...
        students: registrations.filter(r => r.user_type === 'student').length,
                                        ...
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div className="flex-1">
                                                ...
                                                <div className="flex items-center gap-2 mt-2 text-xs text-muted-foreground">
                                                    ...
                                                    <span>•</span>
                                                    <span>{reg.photo_count || 0} photos</span>
                                                    <span>•</span>
                                                    <span>{reg.quality_score || 0}% quality</span>
                                                </div>
                                            </div>
                                        </div>
                                    </CardContent>
